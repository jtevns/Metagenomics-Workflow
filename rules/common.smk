...
def get_refs_to_index(wildcards):
    ref_fasta = assem_info.loc[(wildcards.ref), ["path"]].dropna()
    return(ref_fasta)
        
def get_mapping_outputs(map_scheme):
    outfiles = list()
    with open(map_scheme) as f:
        scheme_yaml = yaml.safe_load(f)
    
    for key in scheme_yaml:
        ref = key
        samples = scheme_yaml[key]
        tmp_outfiles = ["/".join(["mapping",ref,str(x) + "_mapped_sorted.bam.bai"]) for x in samples]
...
def get_binning_output(binning_scheme):
    ...
    outputs = ["binning/" + str(x) + "/bin_fasta_extraction.done" for x in scheme_yaml.keys()]
    return(outputs)

def get_bams(wildcards):
    all_bams = list()
    binning_scheme = config["Mapping_Params"]["scheme"]
    map_info_sub = mapping_info.loc[wildcards.assembly,:].set_index("sample")
    #get list of int fastas to go in an assembly togethor
    with open(binning_scheme) as f:
        scheme_yaml = yaml.load(f, Loader=yaml.FullLoader)
    for sample in scheme_yaml[wildcards.assembly]:
        bam = map_info_sub.loc[(sample), ["bam"]].dropna()
        if len(bam) == 1:
            all_bams += list(bam)
    return(all_bams)
...
def get_reads_for_assem(wildcards):
    reads = list()
    assembly_scheme = config["Assembly_Params"]["scheme"]
    #get list of int fastas to go in an assembly togethor
    with open(assembly_scheme) as f:
        scheme_yaml = yaml.load(f, Loader=yaml.FullLoader)
